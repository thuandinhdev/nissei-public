...
class DatabaseManager
{
    ...
    public function updateDB()
    {
        ...
        try {
            ...
            if ($settings->product_version < 2.2) {
                // --
                // Database connection
                $mysqli = new \mysqli(env('DB_HOST'), env('DB_USERNAME'), env('DB_PASSWORD'), env('DB_DATABASE'));
                // Check for errors
                ...
                $path_to_file = base_path('/public/sql/update_2_2.sql');
                $file_contents = file_get_contents($path_to_file);
                $query = str_replace(
                    "vipspm_",
                    env('TABLE_PREFIX'). '_',
                ...
                $settings->product_version = '2.2';
                $settings->save();
            }

            if ($settings->product_version < 3.0) {
                $settings->product_version = '3.0';
                $settings->save();
            }

            if ($settings->product_version < 4.0) {
                // --
                // Database connection
                $mysqli = new \mysqli(env('DB_HOST'), env('DB_USERNAME'), env('DB_PASSWORD'), env('DB_DATABASE'));
                // Check for errors
                ...
                $path_to_file = base_path('/public/sql/update_4_0.sql');
                $file_contents = file_get_contents($path_to_file);
                $query = str_replace(
                    "vipspm_",
                    env('TABLE_PREFIX'). '_',
